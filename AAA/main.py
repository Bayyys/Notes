...
class MyWindow(QMainWindow):
    ...
    def updateCom(self, port_list): # 更新串口号
        ...
        if glo.get_scan():  # 当前已经连接, 避免重复连接
            if glo.get_com() in [port_list[i][0] for i in range(len(port_list))]:
                self.box_com.setCurrentText(glo.get_com() + ' ' + port_list[[port_list[i][0] for i in range(len(port_list))].index(glo.get_com())][1])
                return
            else:
    ...
    def connect_clicked(self):  # 连接按钮点击事件
        ...
        if serUtil.serialIsOpen(glo.get_ser()):    # 连接成功
            ...
            self.lb_connect.setStyleSheet('color: green')
    ...
    def filePath_clicked(self, Filepath):   # 文件路径按钮点击事件
        path = QFileDialog.getExistingDirectory(None,"选取文件夹", self.et_filePath.text())  # 起始路径
        if path != "":
        ...
        if glo.dataNotEmpty() and glo.get_connected():
            ...
            for i in range(len(y_list)):
                ...
                if i % 20 == 0:
                    ...
                    self.chartFrameList[index].dataAxisX.setMin(x - 10)
