...
- **neuralnet.py** - 是你将在第二部分做所有工作的文件。你会得到一个**NeuralNet**类，它实现了**torch.nn.module**。这个类由**__init__()**、**forward()**和**step()**函数组成。(除了下面的重要细节外，在骨架代码中给出了更多关于 NeuralNet 类中每个方法应该做什么的信息。) 
